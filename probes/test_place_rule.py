...
# Two `show place:` rules around one placement, which is what a rendering that is measured
# rather than laid out puts around the placements it holds.
...
#let outer(body) = {
  show place: it => {
    seen.update(v => v + ((it.at("label", default: none),),))
    it
  }
  body
}
...
#context {
  let names = seen.final().map(it => repr(it.first()))
  assert.eq(names, EXPECTED, message: "the outer rule saw " + repr(names))
}
"""
...
def test_a_nested_rule_runs_first_and_hands_its_label_to_the_rule_around_it(
    typst: TypstRunner,
):
    ...
    """
    typst.ok(
        NESTED.replace("INNER", "show place: it => [#it<unrecorded>]").replace(
            "EXPECTED", '("<unrecorded>", "none")'
        )
    )


def test_labelling_a_placement_changes_no_layout(typst: TypstRunner):
    """The other half of that channel: a measured rendering measures the same with it.

    Without this, suppressing the recording would change the footprint a region reserves,
    which is the very number the measurement exists to produce.
    """
    typst.ok(
        BODY
        + """
#let labelled = {
  show place: it => [#it<unrecorded>]
  body
}
...
#context {
  assert.eq(
    repr(measure(body, width: 300pt)),
    repr(measure(labelled, width: 300pt)),
    message: "the label changed the layout: "
      + repr(measure(body, width: 300pt)) + " became " + repr(measure(labelled, width: 300pt)),
  )
}
"""
    )
