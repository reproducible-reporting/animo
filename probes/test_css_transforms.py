...
The two that matter most are negative: the `transform` shorthand clobbers typst's own
positioning, and `scale` without `transform-box: fill-box` scales about the wrong origin.
...
# the outer group carries typst's own translate, the inner group is the slot CSS writes to.
